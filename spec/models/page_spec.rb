...
RSpec.describe Page, type: :model do
  ...
  describe "scopes" do
    describe ".published" do
      let(:page1) {create(:page, published: true)}
      let(:page2) { create(:page) }

      before do
        [page1, page2]
      end
      it "returns only publised pages" do
  ...
  describe ".by_term" do
    ...
    before {[page1, page2]}
    it "returns page for the given term" do
      expected = [page1, page2, page3]
      expect(Page.by_term("foo")).to match_array(expected)
    end

    it "returns pages for mutiple terms" do
      expected = [page3]
  ...
  describe ".by_year_month" do
    ...
    it "returns pages for given year and month" do
  ...
  describe "#tags_string_for_form" do
    ...
    let(:tag1) {create(:tag, name: 'bar')}
    let(:page) {create(:page, :published)}

    before do
      create(:page_tag, page:, tag:)
      create(:page_tag, page:, tag: tag1)
    end
